...
const App = () => {
  ...
  const getUser = async (id) => {
    try {
      ...
      if (status) {
        ...
        if (JSON.stringify(dataObj) !== JSON.stringify(userDisplayData)) {
          setUserDisplayData(dataObj);
        }
        return dataObj;
      } else {
        return null;
  ...
  const handleUpdateUserCoinsSubmit = async (id) => {
    ...
    if (!isNaN(coinValue)) {
      await handleUpdateUserCoins(id, coinValue);
    } else {
            ...
            <div className="space-y-8">
              ...
              <div className="flex items-center justify-center space-x-4 bg-gradient-to-r from-yellow-100 to-yellow-200 p-4 rounded-lg shadow-lg">
                ...
                <div className="text-gray-800 text-lg font-semibold">
                  Coins:{" "}
                  <span className="text-yellow-600 font-bold">{coin}</span>
                </div>
              </div>
              <div className="flex flex-col items-center space-y-4 p-4 bg-white shadow-lg rounded-lg w-full max-w-md">
                ...
                <button
                  onClick={() => handleUpdateUserCoinsSubmit(profileData.id)}
